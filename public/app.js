...
function showResult(items,kind,blessing=''){
  $('#spinAgain').classList.remove('hidden');
  ...
  $('#resultBlessing').textContent=blessing;$('#resultBlessing').classList.toggle('hidden',!blessing);
...
function confirmChoice(){
  if(spinType==='roomCuisine'||spinType==='roomRestaurant'){$('#resultDialog').close();return;}
...
function showPage(name){if(name!=='room')closeRoomSocket();document.body.dataset.page=name;$$('.page').forEach(p=>p.classList.remove('active'));$(`#${name}Page`).classList.add('active');$$('.nav-btn').forEach(b=>b.classList.toggle('active',b.dataset.page===name));window.scrollTo({top:0,behavior:'smooth'});if(name==='admin')checkSession();if(name==='directory')renderDirectory();}
async function checkSession(){try{admin=(await request('/api/session')).admin;}catch{admin=false;}$('#loginView').classList.toggle('hidden',admin);$('#dashboardView').classList.toggle('hidden',!admin);$('#adminEntry').textContent=admin?'管理小屋':'管理入口';if(admin){await loadAdminBlessings();renderAdmin();loadProgress();}}
...
function openEdit(type,id=''){
  ...
  else if(type==='blessing'){const item=adminBlessings.find(x=>x.id===id);$('#editTitle').textContent=item?'编辑祝福语':'添加祝福语';$('#blessingText').value=item?.text||'';}
...
async function moveBlessing(id,direction){const ordered=[...adminBlessings],from=ordered.findIndex(x=>x.id===id),to=direction==='up'?from-1:from+1;if(from<0||to<0||to>=ordered.length)return;[ordered[from],ordered[to]]=[ordered[to],ordered[from]];try{await request('/api/blessings/reorder',{method:'PUT',body:JSON.stringify({ids:ordered.map(x=>x.id)})});await loadAdminBlessings();await loadState();}catch(e){toast(e.message);}}
...
const roomStorageKey=id=>`kunkun_room_${id}`;
const roomIdFromPath=()=>location.pathname.match(/^\/room\/([A-Za-z0-9_-]{20,40})$/)?.[1]||'';
function inviteUrl(id=currentRoomId){return `${location.origin}/room/${id}`;}
async function shareInvite(url){
  let shared=false;
  if(navigator.share){try{await navigator.share({title:'坤坤邀请你一起吃～',text:'一起排除不想吃的，转出今天的共同美味吧！',url});shared=true;}catch(e){if(e.name!=='AbortError')shared=false;else shared=true;}}
  if(!shared&&navigator.clipboard){try{await navigator.clipboard.writeText(url);toast('邀请链接已经复制啦，发给朋友吧～');shared=true;}catch{}}
  if(!shared){$('#shareFallbackLink').value=url;$('#shareFallbackDialog').showModal();}
}
async function createRoom(){
  try{const created=await request('/api/rooms',{method:'POST',body:'{}'});currentRoomId=created.roomId;const url=inviteUrl();await shareInvite(url);history.pushState({},'',`/room/${currentRoomId}`);showPage('room');await initRoom(currentRoomId);}catch(e){toast(e.message);}
}
async function initRoom(id){
  currentRoomId=id;$('#roomJoinError').textContent='';$('#roomJoinView').classList.remove('hidden');$('#roomView').classList.add('hidden');
  try{await request(`/api/rooms/${id}`);const saved=JSON.parse(localStorage.getItem(roomStorageKey(id))||'null');if(saved?.participantToken&&saved?.nickname){$('#roomNickname').value=saved.nickname;await joinRoom(saved.nickname,saved.participantToken);}}catch(e){$('#roomJoinError').textContent=e.message;}
}
async function joinRoom(nickname,participantToken=''){
  const joined=await request(`/api/rooms/${currentRoomId}/join`,{method:'POST',body:JSON.stringify({nickname,participantToken})});
  roomParticipant={id:joined.participantId,token:joined.participantToken,nickname};localStorage.setItem(roomStorageKey(currentRoomId),JSON.stringify({participantToken:roomParticipant.token,nickname}));currentRoom=joined.room;
  $('#roomJoinView').classList.add('hidden');$('#roomView').classList.remove('hidden');renderRoom();connectRoomSocket();
}
function closeRoomSocket(){clearTimeout(roomReconnectTimer);roomReconnectTimer=null;if(roomSocket){roomSocket.onclose=null;roomSocket.close();roomSocket=null;}}
function connectRoomSocket(){
  closeRoomSocket();if(!currentRoomId||!roomParticipant)return;
  const protocol=location.protocol==='https:'?'wss:':'ws:';roomSocket=new WebSocket(`${protocol}//${location.host}/ws?room=${encodeURIComponent(currentRoomId)}&token=${encodeURIComponent(roomParticipant.token)}`);
  roomSocket.onopen=()=>{roomReconnectAttempts=0;$('#roomNotice').textContent='小草莓信号已连接，大家的选择会实时同步～';};
  roomSocket.onmessage=e=>{let message;try{message=JSON.parse(e.data);}catch{return;}if(message.type==='state'){currentRoom=message.room;renderRoom();applyRoomSpin();}else if(message.type==='error')toast(message.message);};
  roomSocket.onclose=()=>{if(!currentRoomId||document.body.dataset.page!=='room')return;$('#roomNotice').textContent='连接暂时断开，正在重新加入饭局…';const delay=Math.min(1000*2**roomReconnectAttempts++,10000);roomReconnectTimer=setTimeout(connectRoomSocket,delay);};
  roomSocket.onerror=()=>{};
}
function sendRoom(message){if(roomSocket?.readyState===WebSocket.OPEN)roomSocket.send(JSON.stringify(message));else toast('还在连接大家的小草莓信号，请稍等一下～');}
function renderRoom(){
  if(!currentRoom)return;const count=currentRoom.onlineCount||0,strawberries='🍓'.repeat(Math.max(1,count));$('#roomPresence').innerHTML=`<strong>${strawberries}</strong><span>${count||1} 人在线</span>`;
  $('#roomMembers').innerHTML=currentRoom.participants.map(p=>`<article class="room-member seat-${p.seat} ${p.online?'online':'offline'}"><span>🍓</span><div><strong>${escapeHtml(p.nickname)}${p.id===roomParticipant?.id?'（我）':''}</strong><small>${p.online?'在线':'暂时离开'} · 已排除 ${(p.exclusions||[]).length} 个</small></div></article>`).join('');
  const union=new Set(currentRoom.participants.flatMap(p=>p.exclusions||[])),mine=currentRoom.participants.find(p=>p.id===roomParticipant?.id),active=allActiveCuisines(),available=active.filter(c=>!union.has(c.id));
  $('#roomExclusionChips').innerHTML=active.map(c=>{const people=currentRoom.participants.filter(p=>(p.exclusions||[]).includes(c.id)),mineExcluded=(mine?.exclusions||[]).includes(c.id);return `<button class="exclude-chip room-exclude-chip ${people.length?'excluded':''} ${mineExcluded?'mine-excluded':''}" data-room-exclude="${c.id}" data-excluded="${mineExcluded}">${escapeHtml(c.name)}${people.length?`<small>${people.map(p=>escapeHtml(p.nickname)).join('、')}不吃</small>`:''}</button>`;}).join('');
  makeWheel($('#roomCuisineWheel'),available);$('#roomCuisineHint').textContent=available.length?`共同保留 ${available.length} 个菜系`:'总得留点能吃的呀～';
  const spinning=currentRoom.spin&&currentRoom.spin.endsAt>Date.now(),displayStage=spinning&&currentRoom.spin.kind==='cuisine'?'cuisine':currentRoom.stage;
  $('#roomCuisineStage').classList.toggle('hidden',displayStage!=='cuisine');$('#roomRestaurantStage').classList.toggle('hidden',displayStage==='cuisine');
  const cuisine=state.cuisines.find(c=>c.id===currentRoom.selectedCuisineId);$('#roomCuisineName').textContent=cuisine?.name||'';const restaurants=state.restaurants.filter(r=>r.cuisineId===currentRoom.selectedCuisineId);makeWheel($('#roomRestaurantWheel'),restaurants);$('#roomRestaurantEmpty').classList.toggle('hidden',restaurants.length>0);$('#roomRestaurantWheelArea').classList.toggle('hidden',!restaurants.length);
  $('#roomResetRestaurant').classList.toggle('hidden',currentRoom.stage!=='done');
}
...
document.addEventListener('click',e=>{
  const page=e.target.closest('[data-page]')?.dataset.page;if(page){if(roomIdFromPath())history.pushState({},'', '/');showPage(page);}const close=e.target.dataset.close;if(close)$(`#${close}`).close();
  const roomExclude=e.target.closest('[data-room-exclude]');if(roomExclude)sendRoom({type:'exclude',cuisineId:roomExclude.dataset.roomExclude,excluded:roomExclude.dataset.excluded!=='true'});
...
$('#roomSpinCuisine').onclick=$('#roomSpinCuisineBtn').onclick=()=>sendRoom({type:'spin',kind:'cuisine'});$('#roomSpinRestaurant').onclick=$('#roomSpinRestaurantBtn').onclick=$('#roomResetRestaurant').onclick=()=>sendRoom({type:'spin',kind:'restaurant'});$('#roomResetCuisine').onclick=()=>sendRoom({type:'resetCuisine'});
$('#copyFallbackLink').onclick=async()=>{try{await navigator.clipboard.writeText($('#shareFallbackLink').value);$('#shareFallbackDialog').close();toast('邀请链接已经复制啦～');}catch{toast('请长按上面的链接手动复制');}};
window.addEventListener('popstate',()=>{const id=roomIdFromPath();if(id){showPage('room');initRoom(id);}else showPage('home');});
document.body.dataset.page='home';loadState().then(()=>{const id=roomIdFromPath();if(id){showPage('room');initRoom(id);}});
